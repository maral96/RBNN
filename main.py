...
def main():
    args = parse_args()
    global best_prec1, best_prec5, conv_modules
    best_prec1 = 0
    best_prec5 = 0
    ...
    if args.dataset == 'CRC':
        train_image = ImageFolder(
            root= '../../NCT-CRC-HE-100K',
            transform = data_transform_CRC,
        )

        val_image = ImageFolder(
            root='../../CRC-VAL-HE-7K',
            transform=data_transform_CRC,
        )
    ...
    if args.evaluate:
        with torch.no_grad():
            val_loss, val_prec1, val_prec5, time_ = validate(val_loader, model, criterion, 0)
        logging.info('\n Validation Loss {val_loss:.4f} \t'
                     'Validation Prec@1 {val_prec1:.3f} \t'
                     'Validation Prec@5 {val_prec5:.3f} \t'
                     'Inference Time {Inf_time:.8f}\n'
                     .format(val_loss=val_loss, val_prec1=val_prec1, val_prec5=val_prec5, Inf_time=time_))
        return
    ...
    for epoch in range(args.start_epoch+1, args.epochs):
        ...
        train_acc_plot.append(train_prec1)
        val_acc_plot.append(val_prec1)
    ...
    np.save('./train_loss.npy', train_loss_plot, allow_pickle=True)
    np.save('./val_loss.npy', val_loss_plot, allow_pickle=True)
    np.save('./train_acc.npy', train_acc_plot, allow_pickle=True)
    np.save('./val_acc.npy', val_acc_plot, allow_pickle=True)
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None):
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        if args.dataset == 'MNIST':
            input_var = torch.cat((input_var,)*3, axis=1)
        n_imaes += input_var.shape[0]
        ...
        dataset_inference_time += (time.time()- star_time)   
        
        if args.evaluate and i ==0:
            features_t = features.squeeze().cpu().numpy()
            target_t = target.cpu()
        if args.evaluate and i!=0:
            features_t = np.concatenate((features_t, features.squeeze().cpu().numpy()))
            target_t = np.concatenate((target_t, target.cpu()))
        ...
        if i % args.print_freq == 0:
            logging.info('{phase} - Epoch: [{0}][{1}/{2}]\t'
                        ...
                        'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                            ...
                            top1=top1, top5=top5))
    if args.evaluate:
        features_t = np.reshape(features_t, (-1, 512))
        np.save('./features.npy', features_t, allow_pickle=True)
        np.save('./target.npy', target_t, allow_pickle=True)
    print("number_of_imaes", n_imaes)
    return losses.avg, top1.avg, top5.avg, dataset_inference_time
